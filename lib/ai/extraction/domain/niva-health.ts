...
export const NivaHealthSchema = z.object({
  ...
  objecoes: z
    ...
    .describe('Objeções levantadas pelo lead, cada uma como categoria da taxonomia (sem_oportunidade [inclui preço/caro], ficou_na_atual, carencia, rede, concorrente, fora_icp, sem_resposta, timing, reembolso, confianca, decisor, burocracia, outro). NUNCA use fora_da_area nem registro_invalido: quem descobre que a operadora não comercializa na praça é o consultor na cotação — não se deduz da conversa. Array vazio se nenhuma'),
  quer_so_cotacao: z
    .boolean()
    .describe(
...
- objecoes: classifique cada objeção levantada numa categoria da taxonomia: sem_oportunidade (inclui "achou caro"/preço), ficou_na_atual, carencia, rede, concorrente, fora_icp, sem_resposta, timing, reembolso, confianca, decisor, burocracia, outro.
  NUNCA use fora_da_area nem registro_invalido — essa categoria é do consultor (a operadora não comercializa na praça, coisa que só se descobre na cotação). A cidade do lead NÃO é objeção.
